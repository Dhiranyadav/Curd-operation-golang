package main

import (
	"database/sql"
	"log"
	"net/http"
	handler "github.com/usercurd/handler/users"
	service "github.com/usercurd/services/users"
	store "github.com/usercurd/stores/users"

	_ "github.com/go-sql-driver/mysql"
	"github.com/gorilla/mux"
)

func main() {
	db, err := sql.Open("mysql", "root:#Dhiran77@tcp(localhost:3306)/dhiran77")
	if err != nil {
		log.Print("could not open mysql")
	}
	st := store.New(db)
	sr := service.New(st)
	handler := handler.UserHandler{sr}

	r := mux.NewRouter()

	r.HandleFunc("/user", handler.Insert).Methods(http.MethodPost)
	r.HandleFunc("/user/{id}", handler.Update).Methods(http.MethodPut)
	r.HandleFunc("/user/{id}", handler.Delete).Methods(http.MethodDelete)
	r.HandleFunc("/user/{id}", handler.GetById).Methods(http.MethodGet)
	r.HandleFunc("/user", handler.GetAll).Methods(http.MethodGet)
	err = http.ListenAndServe(":3000", r)
	println(err.Error())
}
